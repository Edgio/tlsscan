...
//! ----------------------------------------------------------------------------
missing_ciphersuite_t g_missing_ciphersuites[] = {
        ...
        // -------------------------------------------------------
        // ciphers below reserved for private use (see RFC8446).
        // -------------------------------------------------------
...
//! ----------------------------------------------------------------------------
//! \details: callback set through SSL_set_security_callback() and
...
//! \param:   TODO
//! ----------------------------------------------------------------------------
static SSL_CTX *get_new_CTX(const SSL_METHOD *a_method)
{
        SSL_CTX *ret = SSL_CTX_new(a_method);
        SSL_CTX_set_security_level(ret, 0);
        SSL_CTX_set_security_callback(ret, security_callback_allow_all);
        return ret;
}
//! ----------------------------------------------------------------------------
//! \details: compares list of supported ciphersuites by OpenSSL with complete
//!           list of ciphersuites from IANA.
//!           Marks matches so not re-tested again later.
//! \return:  TODO
//! \param:   TODO
//! ----------------------------------------------------------------------------
...
#define _CIPHERSUITE_LIST_ALL "ALL:COMPLEMENTOFALL"
        STACK_OF(SSL_CIPHER) *l_cipher_list = NULL;
        const SSL_CIPHER *l_cipher = NULL;
        uint32_t l_tls_version = 0;
        uint32_t l_id = 0;
        const SSL_METHOD *l_ssl_methods[] = {
                TLSv1_client_method(),
                TLSv1_1_client_method(),
                TLSv1_2_client_method()
        };
        uint32_t tls_versions[] = { V1_0, V1_1, V1_2 };
        // -------------------------------------------------
        // For each TLS version (not including v1.3)...
        // -------------------------------------------------
        uint32_t l_ssl_method_num = ARRAY_SIZE(l_ssl_methods);
        uint32_t l_arr_len = ARRAY_SIZE(g_missing_ciphersuites);
        for(uint32_t i_m = 0;
            i_m < l_ssl_method_num;
            ++i_m)
        {
                l_tls_version = tls_versions[i_m];
                SSL_CTX *ctx = get_new_CTX(l_ssl_methods[i_m]);
                SSL_CTX_set_cipher_list(ctx, _CIPHERSUITE_LIST_ALL);
                l_cipher_list = SSL_CTX_get_ciphers(ctx);
                // -----------------------------------------
                // Loop through all OpenSSL ciphers...
                // -----------------------------------------
                for(int i_c = 0; i_c < sk_SSL_CIPHER_num(l_cipher_list); ++i_c)
                {
                        l_cipher = sk_SSL_CIPHER_value(l_cipher_list, i_c);
                        l_id = SSL_CIPHER_get_protocol_id(l_cipher);
                        // ---------------------------------
                        // Using the l_cipher ID find match in
                        // IANA list.
                        // ---------------------------------
                        for(uint32_t i_c_m = 0;
                            i_c_m < l_arr_len;
                            ++i_c_m)
                        {
                                if((g_missing_ciphersuites[i_c_m].m_id == l_id) &&
                                    (g_missing_ciphersuites[i_c_m].m_check_tls_versions & l_tls_version))
                                {
                                        // Turn off flag for this version of TLS.
                                        g_missing_ciphersuites[i_c_m].m_check_tls_versions &= ~l_tls_version;
                                }
                        }
                }
